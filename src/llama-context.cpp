...
llama_context::llama_context(
    ...
    if (expert_cache.enabled) {
        const uint32_t top_k = hparams.n_expert_used_max();
...
static bool siliang_moe_route_shape(
    ...
    for (int32_t layer = 0; layer < layer_count; ++layer) {
        ...
        const uint32_t layer_top_k_u = model.hparams.n_expert_used(static_cast<uint32_t>(layer));
        if (layer_top_k_u == 0 || layer_top_k_u > static_cast<uint32_t>(std::numeric_limits<int32_t>::max())) {
            return false;
        }
        const int32_t layer_top_k = static_cast<int32_t>(layer_top_k_u);
        if (expert_count == 0) {
            expert_count = layer_expert_count;
            top_k = layer_top_k;
        } else if (expert_count != layer_expert_count || top_k != layer_top_k) {
            // Siliang currently exposes one route width to the arena/runtime.
            // Reject heterogeneous routed-layer geometry rather than silently
            // applying the wrong K/R/P alignment to a subset of layers.
            return false;
        }
        ++routed_layer_count;
        if (routed_layers) {
...
bool llama_context::siliang_ds4_front_slab_bind(
    ...
    if (model.arch != LLM_ARCH_DEEPSEEK4 || model.hparams.n_layer() != 43 ||
        ...
        model.hparams.n_expert_used_max() != 6 || cparams.pipeline_parallel ||
